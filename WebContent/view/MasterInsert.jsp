...
<div class="container">
	...
	<form action="FileIOServlet" method="post">
		...
		<c:if test="${!empty memberInsertcomplete}">
			<div class="alert alert-warning" role="alert">登録が完了しました 正常系:<%out.print(request.getAttribute("errorCode")); %></div>
		</c:if>
		<c:if test="${!empty memberInserterror}">
			<div class="alert alert-warning" role="alert">処理ができません エラーコード:<%out.print(request.getAttribute("errorCode")); %></div>
		</c:if>
	</form>
	</div>
	<hr>
